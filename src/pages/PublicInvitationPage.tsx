...
const YoutubePlayer: React.FC<{
  ...
  // Ensure enablejsapi=1 for control.
  // Note: changing classes won't reload iframe, but changing 'src' will.
  // We keep 'src' constant.
  const embedUrl = `https://www.youtube.com/embed/${videoId}?autoplay=${autoplay ? 1 : 0}&start=${Math.floor(startSeconds)}&loop=1&playlist=${videoId}&mute=${isMuted ? 1 : 0}&enablejsapi=1&controls=1&rel=0`;
      ...
      >
        {isModalMode && (
          <div className="w-full relative mb-4">
            {/* Close Button defined here or passed in header? Let's use internal close for consistent styling if not passed */}
            <button
              onClick={onClose}
              className="absolute -top-2 -right-2 w-8 h-8 flex items-center justify-center bg-gray-100 rounded-full text-gray-500 hover:text-rose-600 hover:bg-gray-200 transition"
            >
              &times;
            </button>
            {headerContent}
        ...
        <div className={`w-full aspect-video rounded-xl overflow-hidden bg-black shadow-inner ${isModalMode ? 'ring-1 ring-gray-200' : ''}`}>
          <iframe
            width="100%"
            height="100%"
            src={embedUrl}
...
const EffectOverlay: React.FC<{ type: string, color: string }> = ({ type, color }) => {
  ...
  if (type === 'flowery') {
    ...
    return (
      <div className="absolute inset-0 z-20 pointer-events-none overflow-hidden">
        {/* Flowers rolling from Left to Right */}
        {[...Array(15)].map((_, i) => (
          <div
            key={`l2r-${i}`}
            className="absolute animate-flower-roll-right opacity-80"
            style={{
              left: '-10%',
              top: `${Math.random() * 90}%`,
          ...
          >
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-full h-full drop-shadow-md">
              <path d="M12 2C13.1 2 14 2.9 14 4C14 5.1 13.1 6 12 6C10.9 6 10 5.1 10 4C10 2.9 10.9 2 12 2ZM18 10C19.1 10 20 9.1 20 8C20 6.9 19.1 6 18 6C16.9 6 16 6.9 16 8C16 9.1 16.9 10 18 10ZM18 14C19.1 14 20 14.9 20 16C20 17.1 19.1 18 18 18C16.9 18 16 17.1 16 16C16 14.9 16.9 14 18 14ZM12 22C13.1 22 14 21.1 14 20C14 18.9 13.1 18 12 18C10.9 18 10 18.9 10 20C10 21.1 10.9 22 12 22ZM6 14C7.1 14 8 14.9 8 16C8 17.1 7.1 18 6 18C4.9 18 4 17.1 4 16C4 14.9 4.9 14 6 14ZM6 10C7.1 10 8 9.1 8 8C8 6.9 7.1 6 6 6C4.9 6 4 6.9 4 8C4 9.1 4.9 10 6 10ZM12 15C13.66 15 15 13.66 15 12C15 10.34 13.66 9 12 9C10.34 9 9 10.34 9 12C9 13.66 10.34 15 12 15Z" />
            </svg>
          </div>
        ))}

        {/* Flowers rolling from Right to Left */}
        {[...Array(15)].map((_, i) => (
          <div
            key={`r2l-${i}`}
            className="absolute animate-flower-roll-left opacity-80"
            style={{
              right: '-10%',
              top: `${Math.random() * 90}%`,
              width: `${Math.random() * 20 + 15}px`,
...
const InvitationContent: React.FC<{ invitation: Invitation, guestName?: string, isPreview?: boolean, setInv?: React.Dispatch<React.SetStateAction<Invitation | null>> }> = ({ invitation, guestName, isPreview, setInv }) => {
  ...
  return (
    <>
      <SEO
        title={`Walimatulurus ${invitation.groom_name} & ${invitation.bride_name}`}
        description={`Sertai kami meraikan hari bahagia ${invitation.groom_name} & ${invitation.bride_name} pada ${formattedDate}.`}
        image={invitation.settings.background_image}
      />
      <GoogleFontLoader settings={invitation.settings} />

      {invitation.settings.youtube_url && invitation.settings.youtube_show && (
        <YoutubePlayer
          key={isOpen ? 'playing' : 'initial'}
          ...
          headerContent={
            <div className="text-center">
              <h3 className="text-xl font-serif italic font-bold text-gray-800 mb-1">Pemain Muzik</h3>
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold">Kawal Muzik Latar</p>
            </div>
          }
          footerContent={
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 italic text-[10px] text-gray-400 text-center leading-relaxed">
              Mainkan video untuk mendengar lagu.
            </div>
          }
        />
      )}

      {/* Mobile-First Desktop Wrapper - Only in Public View, not in Editor Preview */}
      {!isPreview ? (
        <div className="min-h-screen bg-gray-900 relative flex justify-center">
          {/* Immersive Blurred Background for Desktop sides */}
          <div
            className="fixed inset-0 z-0 opacity-30 blur-3xl scale-110 pointer-events-none"
            style={{
              backgroundImage: `url(${invitation.settings.background_image})`,
              backgroundSize: 'cover',
              backgroundPosition: 'center'
          ...
          <div className="relative w-full max-w-[500px] shadow-[0_0_100px_rgba(0,0,0,0.5)] z-10 min-h-screen">
            <InvitationBody
              ...
              hostStyle={hostStyle}
              invitationTextStyles={invitationTextStyles}
              groomStyleBody={groomStyleBody}
              brideStyleBody={brideStyleBody}
              dateStyleBody={dateStyleBody}
              locationStyleBody={locationStyleBody}
              guestName={guestName}
              showRsvp={showRsvp}
...
const InvitationBody: React.FC<{
  ...
  hostStyle: any;
  invitationTextStyles: any;
  groomStyleBody: any;
  brideStyleBody: any;
  dateStyleBody: any;
  locationStyleBody: any;
  guestName: string | undefined;
  showRsvp: boolean;
  setShowRsvp: (s: boolean) => void;
  isSuccess: boolean;
